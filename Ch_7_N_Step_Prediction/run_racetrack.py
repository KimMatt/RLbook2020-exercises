...
if __name__ == "__main__":
    ...
    for i in range(episodes):
        ...
        reward = -1
        while reward != 1:
            reward = agent.play(mode="v_same")
            t_step += 1
        for t in range(agent.t, agent.t + agent.n - 1):
            if t >= agent.n - 1:
                err = np.sum([agent.rewards[j] + agent.discount_rate * agent.update_values[j+1] - agent.update_values[j] for j in range(t - agent.n + 1, agent.t)])
                update_state = agent.states[t - agent.n + 1]
                agent.values[update_state] += agent.alpha * err
                # G = np.sum([agent.rewards[j] * (agent.discount_rate ** i) for i, j in enumerate([k for k in range(t - agent.n + 1, agent.t)])])
                # update_state = agent.states[t - agent.n + 1]
                # agent.values[update_state] += agent.alpha * (G - agent.values[update_state])
        agent.reset()
        time_step_tracker.append(t_step)

    graph = pd.DataFrame({"time_steps": time_step_tracker}).plot(
        kind="line", title="Exercise 7.2 V_State same")
